...
{
    /// <summary>
    ///  Class for testing ReputationApi
    /// </summary>
    public sealed class ReputationApiTests : ApiTestsBase
    {
        private readonly IReputationApi _instance;

        public ReputationApiTests(): base(Array.Empty<string>())
        {
            _instance = _host.Services.GetRequiredService<IReputationApi>();
        }
        ...
        public async Task GetReputationAsyncTest()
        {
            Client.Option<string> period = default!;
            var response = await _instance.GetReputationAsync(period);
            var model = response.Ok();
            Assert.IsType<GetReputation200Response>(model);
        }

        /// <summary>
        /// Test GetReputationTimeline
        /// </summary>
        [Fact (Skip = "not implemented")]
        public async Task GetReputationTimelineAsyncTest()
        {
            Client.Option<string> period = default!;
            var response = await _instance.GetReputationTimelineAsync(period);
            var model = response.Ok();
            Assert.IsType<GetReputationTimeline200Response>(model);
        }
    }
}
