...
{
    /// <summary>
    ///  Class for testing StoreConnectionsApi
    /// </summary>
    public sealed class StoreConnectionsApiTests : ApiTestsBase
    {
        private readonly IStoreConnectionsApi _instance;

        public StoreConnectionsApiTests(): base(Array.Empty<string>())
        {
            _instance = _host.Services.GetRequiredService<IStoreConnectionsApi>();
        }
        ...
        {
            CreateStoreRequest createStoreRequest = default!;
            var response = await _instance.CreateStoreAsync(createStoreRequest);
            var model = response.Created();
            Assert.IsType<CreateStore201Response>(model);
        }

        /// <summary>
        /// Test DisconnectStore
        /// </summary>
        [Fact (Skip = "not implemented")]
        public async Task DisconnectStoreAsyncTest()
        {
            string storeId = default!;
            var response = await _instance.DisconnectStoreAsync(storeId);
            var model = response.Ok();
            Assert.IsType<DisconnectStore200Response>(model);
        }
        ...
        {
            string storeId = default!;
            var response = await _instance.GetStoreAsync(storeId);
            var model = response.Ok();
            Assert.IsType<CreateStore201Response>(model);
        }
        ...
        {
            var response = await _instance.ListStoresAsync();
            var model = response.Ok();
        ...
        /// </summary>
        [Fact (Skip = "not implemented")]
        public async Task TriggerSyncAsyncTest()
        {
            string storeId = default!;
            Client.Option<string> idempotencyKey = default!;
            var response = await _instance.TriggerSyncAsync(storeId, idempotencyKey);
            var model = response.Ok();
            Assert.IsType<SyncResponse>(model);
        }
    }
}
